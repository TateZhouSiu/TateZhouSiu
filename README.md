...
flowchart LR
  ...
  B --> C[Image and OCR evidence]
  C --> D[Visual layers]
  D --> E[Editable PPTX artifacts]
```

## Stack

`Python` · `Node.js` · `Markdown` · `OCR` · `PowerPoint/PPTX` · `computer vision` · `Codex Agent Skills`

## 中文

我主要在整理 Agent 工作流、图像拆层、OCR 证据链和 PPT 可编辑化这几条技术路径，希望把 AI 生成内容从“一张不可改的图”推进到“结构清晰、可验证、可继续编辑的工程资产”。
